...
def plot_feature_ranking_comparison(lasso_df, rf_df, xgb_df, top_n = 20):
    """
    ...
    ## get the ranks for each variable in each model
    ...
    merged = lasso_df[['feature', 'lasso_rank']].merge(
        rf_df[['feature', 'rf_rank']], on = 'feature', how = 'outer').merge(
            xgb_df[['feature', 'xgb_rank']], on = 'feature', how = 'outer')

    merged['avg_rank'] = merged['avg_rank'] = merged[['lasso_rank', 'rf_rank', 'xgb_rank']].mean(axis=1)
    ...
    for feature in plot_df['feature'].unique():
        ...
        ax.plot(subset['model'], subset['rank'], marker='o', label=feature)
